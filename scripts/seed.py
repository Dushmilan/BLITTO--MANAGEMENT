...
def seed_demo_data(
    ...
    No-op unless demo inventor emails are configured (local dev opt-in).
    """
    now = datetime.now(timezone.utc)

    inv1_email = settings.demo_inventor1_email
    inv1_password = settings.demo_inventor1_password
    inv1_name = settings.demo_inventor1_name or "Demo User One"
    inv2_email = settings.demo_inventor2_email
    inv2_password = settings.demo_inventor2_password
    inv2_name = settings.demo_inventor2_name or "Demo User Two"

    if not inv1_email and not inv2_email:
        return
