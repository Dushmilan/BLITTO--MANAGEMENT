...
def _disclosure(title: str = "Test Patent", inventor_email: str = "inv@uni.edu") -> Disclosure:
    return Disclosure(
        title=title,
...
def test_get_applications_for_inventor_returns_matching() -> None:
    module = _module()
    a1 = module.create_application_shell(_disclosure("Alpha", "a@uni.edu"))
    module.create_application_shell(_disclosure("Beta", "b@uni.edu"))

    results = module.get_applications_for_inventor("a@uni.edu")
...
def test_get_applications_for_inventor_empty_when_no_match() -> None:
    module = _module()
    module.create_application_shell(_disclosure("Alpha", "a@uni.edu"))
    results = module.get_applications_for_inventor("ghost@uni.edu")
    assert results == []
...
def test_change_status_to_valid_state() -> None:
    ...
    updated = module.change_status(app.id, ApplicationStatus.EXAMINATION, "admin@blitto.edu")
...
def test_get_inventors_for_application() -> None:
    ...
    assert inventors[0].inventor_email == "inv@uni.edu"
...
def test_multiple_inventors_on_one_application() -> None:
    ...
    assert inventors[0].inventor_email == "alice@uni.edu"
