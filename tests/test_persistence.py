...
def modules():
    ...
    prosecution = LocalProsecutionModule(document_vault=doc_vault)
...
def seeded_modules(modules):
    ...
    auth.register(RegisterRequest(email="admin@test.local", password="pw"))
    auth.register(RegisterRequest(email="inventor@test.local", password="pw", role="inventor"))

    # Application intake: one application with status change
    app = intake.submit_disclosure(Disclosure(
        title="Test Patent",
        inventors=[Inventor(inventor_name="Dr. Test", inventor_email="inventor@test.local")],
        summary="A test patent",
    ))
    intake.change_status(app.id, ApplicationStatus.FILED, changed_by="admin@test.local")
    ...
    notification.send_notification("inventor@test.local", "Welcome", "Welcome to the system.")

    # Audit: one log entry
    audit.record("test:action", user_id="admin@test.local", application_id=app.id)

    # Filing workflow: a filing record
    filing._records[app.id] = FilingRecord(
        id="rec-1",
        application_id=app.id,
        filed_by="admin@test.local",
    )
...
class TestRoundTrip:
    def test_save_then_load_restores_all_state(self, seeded_modules, tmp_path):
        ...
        admin_user = next(u for u in auth_users.values() if u.email == "admin@test.local")
