...
class TestAuditApiSideEffects:
    ...
    def test_create_application_records_audit(self, client, admin_headers, disclosure_payload):
        audit = client.app.state.audit
        before = len(audit._log)
        resp = client.post("/api/applications", json=disclosure_payload, headers=admin_headers)
    ...
    def test_store_document_records_audit(self, client, admin_headers, disclosure_payload, seed_application):
        # Unlock vault first
    ...
    def test_delete_document_records_audit(self, client, admin_headers, seed_application):
        client.post("/api/vault/unlock", json={"master_key": "master-key"}, headers=admin_headers)
        app_id = seed_application["id"]
        store = client.post(
    ...
    def test_change_status_records_audit(self, client, admin_headers, seed_application):
        app_id = seed_application["id"]
    ...
    def test_change_status_rejected_records_audit(self, client, admin_headers, seed_application):
        app_id = seed_application["id"]
        audit = client.app.state.audit
    ...
    def test_send_notification_records_audit(self, client, admin_headers, seed_application):
        app_id = seed_application["id"]
        audit = client.app.state.audit
        before = len(audit._log)
        resp = client.post(
    ...
    def test_non_mutating_route_does_not_record_audit(self, client, admin_headers, seed_application):
        audit = client.app.state.audit
        before = len(audit._log)
    ...
    def test_audit_entry_includes_user_id(self, client, admin_headers, disclosure_payload):
        audit = client.app.state.audit
        resp = client.post("/api/applications", json=disclosure_payload, headers=admin_headers)
        assert resp.status_code == 200
        latest = list(audit._log.values())[-1]
    ...
    def test_for_application_returns_matching_entries(self, client, admin_headers, disclosure_payload):
        resp = client.post("/api/applications", json=disclosure_payload, headers=admin_headers)
        app_id = resp.json()["id"]
        audit = client.app.state.audit
    ...
    def test_by_actor_returns_user_entries(self, client, admin_headers, disclosure_payload):
        client.post("/api/applications", json=disclosure_payload, headers=admin_headers)
        audit = client.app.state.audit
