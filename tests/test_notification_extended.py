...
def test_send_notification_to_multiple_recipients() -> None:
    module = _module()
    n1 = module.send_notification("a@pdn.ac.lk", "Subject A", "Body A")
    n2 = module.send_notification("b@pdn.ac.lk", "Subject B", "Body B")
...
def test_get_for_recipient_empty_when_none() -> None:
    module = _module()
    assert module.get_for_recipient("ghost@pdn.ac.lk") == []
...
def test_notification_has_unique_ids() -> None:
    module = _module()
    n1 = module.send_notification("a@pdn.ac.lk", "S1", "B1")
    n2 = module.send_notification("b@pdn.ac.lk", "S2", "B2")
    assert n1.id != n2.id


def test_send_notification_with_empty_body() -> None:
    module = _module()
    n = module.send_notification("a@pdn.ac.lk", "Empty Body", "")
    assert n.body == ""
    assert n.recipient_email == "a@pdn.ac.lk"


def test_send_status_change_creates_notification() -> None:
    module = _module()
    n = module.send_status_change("inv@pdn.ac.lk", "app-1", "EXAMINATION")
    assert n.recipient_email == "inv@pdn.ac.lk"
    assert "EXAMINATION" in n.body


def test_get_for_recipient_filters_correctly() -> None:
    module = _module()
    module.send_notification("a@pdn.ac.lk", "S1", "B1")
    module.send_notification("b@pdn.ac.lk", "S2", "B2")
    a_notifs = module.get_for_recipient("a@pdn.ac.lk")
    assert len(a_notifs) == 1
    assert a_notifs[0].recipient_email == "a@pdn.ac.lk"
