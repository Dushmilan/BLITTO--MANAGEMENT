...
def _disclosure() -> Disclosure:
    return Disclosure(inventor_name="A", inventor_email="a@uni.edu", title="X", summary="s")
...
def test_change_status_records_history() -> None:
    ...
    updated = intake.change_status(app.id, ApplicationStatus.EXAMINATION, "admin@blitto.edu")
    assert updated is not None
    assert updated.status == ApplicationStatus.EXAMINATION
    # Unknown application returns None.
    assert intake.change_status("nope", ApplicationStatus.GRANTED, "admin") is None


def test_token_expires() -> None:
    module = LocalAuthorizationModule()
    module.register(RegisterRequest(email="a@blitto.edu", role=Role.INVENTOR))
    token = module.issue_token("a@blitto.edu")
    ...
    expired = jwt.encode(
        {"sub": "x", "email": "a@blitto.edu", "role": "inventor",
